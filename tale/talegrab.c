...
int nemotale_prepare_grab(struct talegrab *grab, struct nemotale *tale, uint64_t device, nemotale_dispatch_grab_t dispatch)
{
	grab->tale = tale;
	grab->device = device;
...
struct talegrab *nemotale_create_grab(struct nemotale *tale, uint64_t device, nemotale_dispatch_grab_t dispatch)
{
	...
	nemotale_prepare_grab(grab, tale, device, dispatch);

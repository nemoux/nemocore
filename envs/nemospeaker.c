...
static void nemospeaker_dispatch_show_resize(struct nemoshow *show, int32_t width, int32_t height)
{
	...
	nemoshow_view_set_scope(show, "e:%f,%f:%f,%f", width * 0.05f, height * 0.05f, width * 0.45f, height * 0.45f);
}
...
struct nemospeaker *nemospeaker_create(struct nemoshell *shell, uint32_t size, double x, double y, double r)
{
	...
	speaker->layout0 = one = nemoshow_item_create(NEMOSHOW_PATHTWICE_ITEM);
	...
	nemoshow_item_path_load_svg(one, NEMOENVS_RESOURCES "/speaker/speaker-1.svg", 0.0f, 0.0f, width, height);

	speaker->layout1 = one = nemoshow_item_create(NEMOSHOW_PATHTWICE_ITEM);
	nemoshow_one_attach(group, one);
	nemoshow_item_set_width(one, width);
	nemoshow_item_set_height(one, height);
	nemoshow_item_set_fill_color(one, 0x1e, 0xdc, 0xdc, 0xff);
	nemoshow_item_set_alpha(one, 0.0f);
	nemoshow_item_path_load_svg(one, NEMOENVS_RESOURCES "/speaker/speaker-2.svg", 0.0f, 0.0f, width, height);

	speaker->layout2 = one = nemoshow_item_create(NEMOSHOW_PATHTWICE_ITEM);
	nemoshow_one_attach(group, one);
	nemoshow_item_set_width(one, width);
	nemoshow_item_set_height(one, height);
	nemoshow_item_set_fill_color(one, 0xff, 0x8c, 0x32, 0xff);
	nemoshow_item_set_alpha(one, 0.0f);
	nemoshow_item_path_load_svg(one, NEMOENVS_RESOURCES "/speaker/speaker-3.svg", 0.0f, 0.0f, width, height);
	...
	nemoshow_view_set_scope(show, "e:%f,%f:%f,%f", size * 0.05f, size * 0.05f, size * 0.45f, size * 0.45f);

...
int nemometro_intersect_cube(float *cube, float *o, float *d, float *mint, float *maxt)
{
	...
	if (tmin >= 0.0f) {
		if (nmin[0] == 1.0f)
			plane = NEMOMETRO_RIGHT_PLANE;
		else if (nmin[0] == -1.0f)
			plane = NEMOMETRO_LEFT_PLANE;
		else if (nmin[1] == 1.0f)
			plane = NEMOMETRO_TOP_PLANE;
		else if (nmin[1] == -1.0f)
			plane = NEMOMETRO_BOTTOM_PLANE;
		else if (nmin[2] == 1.0f)
			plane = NEMOMETRO_FRONT_PLANE;
		else if (nmin[2] == -1.0f)
			plane = NEMOMETRO_BACK_PLANE;

...
static int nemotoyz_path_svg_get_transform_args(struct nemotoken *token, int offset, double *args)
{
	const char *value;
	int i;
...
static int nemotoyz_path_svg_get_transform(SkMatrix *matrix, const char *value)
{
	...
	while (i < count) {
		...
		if (type != NULL) {
			if (strcmp(type, "translate") == 0) {
				nargs = nemotoyz_path_svg_get_transform_args(token, i, args);
				if (nargs == 1) {
...
void nemotoyz_path_svg(struct toyzpath *path, const char *url, float x, float y, float w, float h)
{
	...
	nemolist_for_each(node, &xml->nodes, nodelink) {
		...
		if (strcmp(node->name, "path") == 0) {
			...
			if (d != NULL)
				SkParsePath::FromSVGString(d, &npath);
			...
			npath.addRect(x, y, x + width, y + height);
		...
		if ((attr0 = nemoxml_node_get_attr(node, "transform")) != NULL) {
			nemotoyz_path_svg_get_transform(&smatrix, attr0);

			npath.transform(smatrix);
		}

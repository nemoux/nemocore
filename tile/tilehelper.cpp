...
struct tileone *nemotile_one_create_polygon(const char *filepath, const char *basepath)
{
	struct tileone *one;
	...
	for (i = 0, idx = 0; i < shapes.size(); i++) {
		for (j = 0; j < shapes[i].mesh.indices.size() / 3; j++, idx++) {
			...
			if (shapes[i].mesh.normals.size() > 0) {
				...
				nemovector_sub_xyz(s0, p1, p0);
				nemovector_sub_xyz(s1, p2, p0);
				nemovector_cross_xyz(s0, s0, s1);
				nemovector_normalize_xyz(s0);
...
struct tileone *nemotile_one_create_polyline(const char *filepath, const char *basepath)
{
	struct tileone *one;
	struct nemomatrix transform;
	std::vector<tinyobj::shape_t> shapes;
	...
	for (i = 0; i < shapes.size(); i++) {
		one->count += shapes[i].mesh.indices.size() * 2;
	}
	...
	for (i = 0, idx = 0; i < shapes.size(); i++) {
		for (j = 0; j < shapes[i].mesh.indices.size() / 3; j++, idx++) {
			...
			one->vertices[idx * 18 + 0] = p0[0];
			one->vertices[idx * 18 + 1] = p0[1];
			one->vertices[idx * 18 + 2] = p0[2];
			one->vertices[idx * 18 + 3] = p1[0];
			one->vertices[idx * 18 + 4] = p1[1];
			one->vertices[idx * 18 + 5] = p1[2];
			one->vertices[idx * 18 + 6] = p0[0];
			one->vertices[idx * 18 + 7] = p0[1];
			one->vertices[idx * 18 + 8] = p0[2];
			one->vertices[idx * 18 + 9] = p2[0];
			one->vertices[idx * 18 + 10] = p2[1];
			one->vertices[idx * 18 + 11] = p2[2];
			one->vertices[idx * 18 + 12] = p1[0];
			one->vertices[idx * 18 + 13] = p1[1];
			one->vertices[idx * 18 + 14] = p1[2];
			one->vertices[idx * 18 + 15] = p2[0];
			one->vertices[idx * 18 + 16] = p2[1];
			one->vertices[idx * 18 + 17] = p2[2];

			one->diffuses[idx * 24 + 0] = r;
			one->diffuses[idx * 24 + 1] = g;
			one->diffuses[idx * 24 + 2] = b;
			one->diffuses[idx * 24 + 3] = 1.0f;
			one->diffuses[idx * 24 + 4] = r;
			one->diffuses[idx * 24 + 5] = g;
			one->diffuses[idx * 24 + 6] = b;
			one->diffuses[idx * 24 + 7] = 1.0f;
			one->diffuses[idx * 24 + 8] = r;
			one->diffuses[idx * 24 + 9] = g;
			one->diffuses[idx * 24 + 10] = b;
			one->diffuses[idx * 24 + 11] = 1.0f;
			one->diffuses[idx * 24 + 12] = r;
			one->diffuses[idx * 24 + 13] = g;
			one->diffuses[idx * 24 + 14] = b;
			one->diffuses[idx * 24 + 15] = 1.0f;
			one->diffuses[idx * 24 + 16] = r;
			one->diffuses[idx * 24 + 17] = g;
			one->diffuses[idx * 24 + 18] = b;
			one->diffuses[idx * 24 + 19] = 1.0f;
			one->diffuses[idx * 24 + 20] = r;
			one->diffuses[idx * 24 + 21] = g;
			one->diffuses[idx * 24 + 22] = b;
			one->diffuses[idx * 24 + 23] = 1.0f;

			one->normals[idx * 18 + 0] = n0[0];
			one->normals[idx * 18 + 1] = n0[1];
			one->normals[idx * 18 + 2] = n0[2];
			one->normals[idx * 18 + 3] = n1[0];
			one->normals[idx * 18 + 4] = n1[1];
			one->normals[idx * 18 + 5] = n1[2];
			one->normals[idx * 18 + 6] = n0[0];
			one->normals[idx * 18 + 7] = n0[1];
			one->normals[idx * 18 + 8] = n0[2];
			one->normals[idx * 18 + 9] = n2[0];
			one->normals[idx * 18 + 10] = n2[1];
			one->normals[idx * 18 + 11] = n2[2];
			one->normals[idx * 18 + 12] = n1[0];
			one->normals[idx * 18 + 13] = n1[1];
			one->normals[idx * 18 + 14] = n1[2];
			one->normals[idx * 18 + 15] = n2[0];
			one->normals[idx * 18 + 16] = n2[1];
			one->normals[idx * 18 + 17] = n2[2];
